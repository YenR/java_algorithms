...
public class Main {
	...
	protected static void run(ETSPPCInstance instance) throws Exception {
		...
		long offs = end - start;
		long timeout = 60000; // 30 Sekunden
		...
		if (Math.abs(sumDistance - upper_bound) > upper_bound * 0.00001) {
		...
		if (upper_bound - originalInstance.getThreshold() > originalInstance.getThreshold() * 0.00001)
